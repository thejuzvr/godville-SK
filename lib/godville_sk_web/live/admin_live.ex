defmodule GodvilleSkWeb.AdminLive do
  ...
  def mount(_params, _session, socket) do
    ...
    if user.email != "admin@admin.ru" do
      {:ok, push_navigate(socket, to: "/dashboard") |> put_flash(:error, "Доступ запрещен. Только для Имперской Канцелярии.")}
    else
    ...
    <div class="min-h-screen bg-[#0a0a0a] text-[#00ff41] font-mono p-4 md:p-8 selection:bg-[#00ff41] selection:text-[#0a0a0a]">
      ...
      <div class="max-w-6xl mx-auto space-y-6 relative z-10">
        ...
        <header class="border-b-2 border-[#00ff41]/30 pb-4 flex justify-between items-end">
          <div>
            <h1 class="text-3xl font-black tracking-tighter uppercase italic">Imperial_Overseer_v1.0</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-1 space-y-6">
            <section class="border border-[#00ff41]/20 bg-[#0f0f0f] p-4 shadow-[0_0_20px_rgba(0,255,65,0.05)]">
              <h2 class="text-xs font-bold mb-4 border-b border-[#00ff41]/20 pb-1 uppercase tracking-widest">State_Monitor</h2>
              <div class="space-y-2 text-sm">
                ...
                <div class="space-y-3 pt-2">
                  <.debug_bar label="HEALTH" val={@hero_state.hp} max={@hero_state.max_hp} color="bg-red-500" />
                  <.debug_bar label="STAMINA" val={@hero_state.stamina} max={@hero_state.stamina_max} color="bg-green-500" />
                  <.debug_bar label="POWER" val={@hero_state.intervention_power} max={100} color="bg-blue-500" />
            ...
            <section class="border border-[#00ff41]/20 bg-[#0f0f0f] p-4">
              <h2 class="text-xs font-bold mb-4 border-b border-[#00ff41]/20 pb-1 uppercase tracking-widest">Active_Target</h2>
              <%= if @hero_state.target do %>
                 <div class="text-sm space-y-1">
                   <div>NAME: <span class="text-red-500 font-bold"><%= @hero_state.target.name %></span></div>
                   <div>HP: <%= @hero_state.target[:hp] || "N/A" %></div>
                   <button phx-click="kill_target" class="mt-2 w-full py-1 text-xs bg-red-900/30 border border-red-500/50 hover:bg-red-500 hover:text-black transition-all uppercase font-bold">Insta_Kill</button>
                 </div>
              <% else %>
          ...
          <div class="lg:col-span-2 space-y-6">
            <section class="border border-[#00ff41]/20 bg-[#0f0f0f] p-4">
              <h2 class="text-xs font-bold mb-6 border-b border-[#00ff41]/20 pb-1 uppercase tracking-widest">Inject_Parameters</h2>
              
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <!-- Status Set -->
                <div class="space-y-4">
                  <div>
                    <label class="text-[10px] block mb-1 opacity-50 uppercase">FORCED_STATUS_OVERRIDE</label>
                  ...
                  <div class="pt-4">
                    <button phx-click="force_tick" class="w-full py-3 bg-[#00ff41]/10 border-2 border-[#00ff41] text-[#00ff41] hover:bg-[#00ff41] hover:text-black transition-all font-black uppercase text-lg flex items-center justify-center gap-2 group">
            ...
            <!-- Inventory Manager -->
            <section class="border border-[#00ff41]/20 bg-[#0f0f0f] p-4">
              <h2 class="text-xs font-bold mb-4 border-b border-[#00ff41]/20 pb-1 uppercase tracking-widest">Payload_Manifest (Inventory)</h2>
              <div class="flex gap-2 mb-4">
                <input type="text" phx-keyup="update_item_name" value={@debug_item} placeholder="ITEM_ID_OR_NAME..." class="flex-1 bg-black border border-[#00ff41]/30 p-1 text-sm focus:outline-none focus:border-[#00ff41]" />
                <button phx-click="add_item" class="px-4 py-1 bg-[#00ff41]/20 border border-[#00ff41] text-xs font-bold uppercase hover:bg-[#00ff41] hover:text-black">Inject</button>
              </div>
              <div class="flex flex-wrap gap-2">
                <%= for item <- @hero_state.inventory do %>
                  <div class="group relative px-2 py-0.5 bg-black border border-[#00ff41]/20 text-[10px] flex items-center gap-2">
                    <span><%= item %></span>
                    <button phx-click="remove_item" phx-value-item={item} class="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                  </div>
                <% end %>
              </div>
            </section>
            ...
            <section class="border border-[#00ff41]/20 bg-[#0f0f0f] p-4 h-48 flex flex-col">
              <h2 class="text-xs font-bold mb-2 border-b border-[#00ff41]/20 pb-1 uppercase tracking-widest">Session_Logs</h2>
              <div id="admin-log" class="flex-1 overflow-y-auto text-[10px] opacity-70 space-y-1">
                <%= for msg <- Enum.reverse(@log) do %>
                  <div class="flex gap-2">
                    <span class="opacity-30">[<%= NaiveDateTime.utc_now() |> to_string() |> String.slice(11..18) %>]</span>
                    <span class={if String.contains?(msg, "ERROR"), do: "text-red-500", else: ""}> > <%= msg %></span>
                  </div>
                <% end %>
              </div>
            </section>
  ...
  def debug_bar(assigns) do
    ...
    <div>
      ...
      <div class="w-full h-1.5 bg-black border border-[#00ff41]/10">
        <div class={"h-full #{@color} shadow-[0_0_5px_rgba(var(--tw-shadow-color),0.5)]"} style={"width: #{min(100, (@val/@max)*100)}%"}></div>
      </div>
    </div>
    """
  end
  ...
  def debug_input(assigns) do
    ...
    <div>
      ...
      <form phx-submit={@event} class="flex gap-1">
        <input type={@type} name="value" class="w-full bg-black border border-[#00ff41]/30 p-1 text-xs focus:outline-none focus:border-[#00ff41]" />
        <button type="submit" class="bg-[#00ff41]/10 border border-[#00ff41]/50 px-2 hover:bg-[#00ff41] hover:text-black transition-all">OK</button>
      </form>
    </div>
    """
  end
  ...
  def handle_event("set_status", %{"status" => status}, socket) do
    ...
    updates = if status_atom == :sovngarde do
      %{
        status: :sovngarde,
  ...
  def handle_event("add_item", _, socket) do
    ...
    if item != "" do
      ...
      {:noreply, socket |> assign(debug_item: "") |> add_log("INJECT: Item #{item} added to manifest.")}
    else
      {:noreply, socket}
    end
  end
  ...
  def handle_event("kill_target", _, socket) do
    if socket.assigns.hero_state.target do
      Hero.debug_update(socket.assigns.hero.name, %{target: Map.put(socket.assigns.hero_state.target, :hp, 0)})
