      ...
      <main class="flex-1 overflow-y-auto p-6 max-w-5xl mx-auto w-full">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section class="bg-card/30 border border-border/50 p-6 rounded-lg backdrop-blur-sm">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="flex gap-3 text-xs">
                <span class="text-foreground/50">
                  <%= length(@hero_state.inventory) %> / <%= @hero_state.inventory_capacity %>
                </span>
                <span class={
                  if @hero_state.overload_penalty < 0,
                    do: "text-red-400 font-bold",
                    else: "text-foreground/40"
                }>
                  Вес: <%= round(@hero_state.inventory_weight * 10) / 10 %>/50 кг <%= if @hero_state.overload_penalty <
                                                                                           0,
                                                                                         do: " (-2)",
                                                                                         else: "" %>
                </span>
            ...
            <div class="h-48 overflow-y-auto mb-4 pr-2">
        ...
        <div class="bg-card border border-primary/30 w-full max-w-sm overflow-hidden rounded-lg shadow-2xl animate-in fade-in zoom-in duration-200">
          ...
          <div class="p-6">
            <div class="flex justify-between items-start mb-4">
              <div>
                <h3 class="font-headline text-primary text-2xl truncate">
                  <%= @selected_item.name %>
                </h3>
              ...
              <button
                ...
                class="text-foreground/40 hover:text-primary transition-colors"
              >
            ...
            <div class="space-y-4">
              ...
              <div class="flex gap-3 pt-2">
                ...
                <button
                  phx-click="close_modal"
                  class="flex-1 py-2 border border-border/50 text-foreground/70 hover:bg-foreground/5 font-headline text-sm transition-all"
                >
                  Закрыть
                </button>
              </div>
    ...
    >
      <%= if @item_name do %>
        <div class="text-primary animate-in fade-in duration-500">
          <svg
            class="w-8 h-8 opacity-40 absolute"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="1"
          ...
          <span class="text-[8px] font-bold text-center relative z-10 px-1 truncate w-12">
            <%= @item_name %>
          </span>
        </div>
        <button
      ...
      <% else %>
        <svg
          class="w-6 h-6 text-foreground/10"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          stroke-width="1.5"
